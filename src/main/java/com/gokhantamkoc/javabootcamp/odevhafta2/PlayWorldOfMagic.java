...
public class PlayWorldOfMagic implements CommandLineRunner {
	...
	public static int resolveBattle(
		...
		float totalBossHp = 0;
		float selectspellDamage= 0;

		for(int i = 0; i<bossHPs.length;i++){
			totalBossHp += bossHPs[i];
		}

		for(int j = 0; j< spellDamageInfo.length; j++){
			if(selectspellDamage < spellDamageInfo[j]){
				selectspellDamage = spellDamageInfo[j];
			}
		}
		System.out.println("TOTAL BOSS HP = " + totalBossHp);
		System.out.println("MAX DAMAGE = " + selectspellDamage);
		while(totalBossHp > 0){
			totalBossHp -= selectspellDamage;
			spellsUsed++;
		}

		System.out.println("SPELL USED = " + spellsUsed);
